...
<title>Admin Lecturer Profile</title>
...
<script type="text/javascript" src="${pageContext.request.contextPath }/resources/js/sideCollapse.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath }/resources/js/sideBarCollapse.js"></script>
...
<br><br>
        ...
        <h1 class="h2 mb-0 text-gray-800 text-center">AITI-KACE LECTURER DASHBOARD</h1>
   ...
   <br><br>
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
           ...
           <div class="container">
		<button type="button" class="btn btn-info float-left" data-toggle="modal" data-target="#myModal">Add lecturer</button>
		...
		<div class="modal fade" id="myModal" role="dialog">
			<div class="modal-dialog">
				...
				<div class="modal-content">
					<div class="modal-header">
					<h4 class="modal-title text-primary">ADD NEW LECTURER</h4>
					...
					<div class="modal-body">
						...
						<form action="${pageContext.request.contextPath }/aitiKaceLecturersApiConsumption/saveLecturer=SaMEmE=HsuSGS-MlsjHS-DN-aDDF-Fdddde-sAmKoG=SmO-sLnINbds=LonCrshs-bBidbdd" method="post">
							...
							<div class="modal-body">
								...
								<div class="form-group">
									<label>LECTURER NAME</label> <input name="name"
										type="text" class="form-control" id="name" value="" required>
								...
								<div class="form-group">
									<label>PHONE</label> <input name="phone" type="text"
										class="form-control checking_phone" id="phone" value="" required>
								...
								<div class="form-group">
									<label>COURSE HANDLE</label> <input name="course_handle"
										type="text" class="form-control" id="course_handle" value="" placeholder="DIPLOMA IN BUSINESS COMPUTING" required>
										<span id ="course_handle_error_message" style="color:red;"></span>
								</div>
...
<div class="card shadow mb-4">
  <div class="card-header py-3">
    <h6 class="m-0 font-weight-bold text-primary">AITI-KACE LECTURER DATATABLE</h6>
  </div>
  <div class="card-body">
	...
	<tbody>
			...
			<c:forEach items="${allAitiKaceLecturers}" var="allAitiKaceLecturers">

				<c:url value="getAitiKaceLecturerById=SaMEmE=HsuSGS-MlsjHS-DN-aDDF-Fdddde-sAmKoG=SmO-sLnINbds=LonCrshs-bBidbdd" var="updateURL">
					<c:param name="id" value="${allAitiKaceLecturers.id }"></c:param>
					<c:param name="name" value="${allAitiKaceLecturers.name }"></c:param>
					<c:param name="email" value="${allAitiKaceLecturers.email }"></c:param>
					<c:param name="phone" value="${allAitiKaceLecturers.phone }"></c:param>
					<c:param name="course_handle" value="${allAitiKaceLecturers.course_handle }"></c:param>
					<c:param name="password" value="${allAitiKaceLecturers.password }"></c:param>
					...
					<c:url value="deleteLecturerInfo=VIEw=deLETeCtuRER-hBSIS=lEctUREr-GbsGhHS=PAGE-hGhgKg=VGSN=Vhanxjjj-hsdSBDSHNDHNNDH-Gh=yOPPJD" var="deleteURL">
					<c:param name="id" value="${allAitiKaceLecturers.id }"></c:param>
				</c:url>
				...
				<tr>
					...
					<td>${allAitiKaceLecturers.id }</td>
					<td>${allAitiKaceLecturers.name }</td>
					<td>${allAitiKaceLecturers.email }</td>
					<td>${allAitiKaceLecturers.phone }</td>
					<td>${allAitiKaceLecturers.course_handle }</td>
					<td>${allAitiKaceLecturers.password }</td>
					<td>${allAitiKaceLecturers.reg_date }</td>
					<td>${allAitiKaceLecturers.update_time }</td>
...
$(document).ready(function () {
    ...
    $('.checking_email').keyup(function (e) {
        ...
        $.ajax({
            type: "POST",
            url: "getLecturerLiveValidationOnEmail",
            data: {
                "check_submit_btn": 1,
                "email": email,
            },
...
$(document).ready(function () {
    ...
    $('.checking_phone').keyup(function (e) {
        ...
        $.ajax({
            type: "POST",
            url: "getLecturerLiveValidationOnPhoneNUmber",
            data: {
...
function validate(){
	...
	var course_handle = document.getElementById("course_handle").value;
	...
	if (name == ""){
        document.getElementById("name_error_message").innerHTML="** Lecturer Name required";
        return false;
    }
	else if (nametype.test(name) == false){
        document.getElementById("name_error_message").innerHTML="** Lecturer Name should be only Characters";
        return false;
    }
