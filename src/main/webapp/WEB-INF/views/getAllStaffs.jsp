...
<br><br>
        ...
        <h1 class="h2 mb-0 text-gray-800 text-center">AITI-KACE STAFFS DASHBOARD</h1>
 ...
 <div class="col">
								<div class="form-group">
									...
									<select name="usertype" id="usertype" class="form-control" onchange="showCoursesModlues(this.id, 'module')" required="required">
       ...
       <div class="form-group">
 			 <label>ASSIGN COURSE TO STAFF</label> 
  	<select name="course" id=module class="form-control" required="required">
...
<!-- 
			...
			<c:forEach items="${getCourseNameAndCourseLevel}" var="getCourseNameAndCourseLevel">			
				
<option value="${getCourseNameAndCourseLevel.course_name}">


			
	${getCourseNameAndCourseLevel.course_name }---${getCourseNameAndCourseLevel.course_level}
...
<option value="Deactivate">Deactivate</option>		     
        ...
        <div class="container">
		<button type="button" hidden="true" class="btn btn-info float-left" data-toggle="modal" data-target="#courseAndModule">Add Course and Module</button>
...
<table id="examples" class="table table-striped table-bordered"  width="100%" cellspacing="0">
	...
	<tbody>
			...
			<c:forEach items="${retrievaAllStaffs}" var="retrievaAllStaffs">

				<c:url value="updateCoursesModules" var="updateCoursesModulesURL">
				<c:param name="name" value="${retrievaAllStaffs.name }"></c:param>	
					<c:param name="staff_id" value="${retrievaAllStaffs.staff_id }"></c:param>
					<c:param name="email" value="${retrievaAllStaffs.email }"></c:param>
					<c:param name="contact" value="${retrievaAllStaffs.contact }"></c:param>
					<c:param name="batch_id" value="${retrievaAllStaffs.batch_id }"></c:param>
					<c:param name="department" value="${retrievaAllStaffs.department }"></c:param>
					<c:param name="status" value="${retrievaAllStaffs.status }"></c:param>
					<c:param name="created_at" value="${retrievaAllStaffs.created_at }"></c:param>
					<c:param name="updated_at" value="${retrievaAllStaffs.updated_at }"></c:param>

				</c:url>
	

	
					<c:url value="deleteCoursesModulesURL" var="deleteCoursesModulesURL">
					<c:param name="id" value="${retrievaAllStaffs.id }"></c:param>
				</c:url>
				...
				<tr>
					...
					<td>${retrievaAllStaffs.id }</td>
					<td>${retrievaAllStaffs.staff_id }</td>
					<td>${retrievaAllStaffs.name }</td>
					<td>${retrievaAllStaffs.email }</td>
					<td>${retrievaAllStaffs.contact }</td>
					<td>${retrievaAllStaffs.batch_id }</td>
					<td>${retrievaAllStaffs.department }</td>
					<td>${retrievaAllStaffs.status }</td>
					<td>${retrievaAllStaffs.created_at }</td>
					<td>${retrievaAllStaffs.updated_at }</td>
...
<div class="card shadow mb-4">
  ...
  <div class="card-body">
	...
	<tbody>
			...
			<c:forEach items="${findAllCoursesModules}" var="findAllCoursesModules">

	

	
					<c:url value="deleteCoursesModules" var="deleteCoursesModulesURL">
					<c:param name="id" value="${findAllCoursesModules.id }"></c:param>
				</c:url>


				<tr>
				
					<td>${findAllCoursesModules.id }</td>
					<td>${findAllCoursesModules.name }</td>
					<td>${findAllCoursesModules.batch_id }</td>
					<td>${findAllCoursesModules.course_level }</td>
					<td>${findAllCoursesModules.module_name }</td>
					<td>${findAllCoursesModules.course_start_date }</td>
					<td>${findAllCoursesModules.course_end_date }</td>
...
function showCoursesModlues(s1,s2) {
	...
	if(s1.value == "Instructor") {
		
    //var optionArray = url: "getAllAitiKaceStudentNames";
	var optionArray = ['Select Course|Select Course', 'CERTIFICATE IN SOFTWARE DEVELOPMENT|CERTIFICATE IN SOFTWARE DEVELOPMENT', 'CISCO CERTIFIED NETWORK ASSOCIATE|CISCO CERTIFIED NETWORK ASSOCIATE',  'CISCO CERTIFIED NETWORK PROFESSIONALS|CISCO CERTIFIED NETWORK PROFESSIONALS',  'CYBER SECURITY|SCYBER SECURITY',  'INTERNATIONAL COMPUTER DRIVING LICENCE|INTERNATIONAL COMPUTER DRIVING LICENCE', 'NETWORK INFORMATION SECURITY|NETWORK INFORMATION SECURITY', 'CERTIFIED INFORMATION SYSTEM AUDITOR|CERTIFIED INFORMATION SYSTEM AUDITOR', 'DIPLOMA IN BUSINESS COMPUTING|DIPLOMA IN BUSINESS COMPUTING', 'COMPUTING INFRASTRUCTURE SYSTEMS & SECURITY|COMPUTING INFRASTRUCTURE SYSTEMS & SECURITY'];	
