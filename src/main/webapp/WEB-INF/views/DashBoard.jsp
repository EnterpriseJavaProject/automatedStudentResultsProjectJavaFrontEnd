  ...
  <!-- End of Topbar -->
   ...
   <!-- Begin Page Content -->
        <div class="container-fluid">
        		...
        		<c:forEach items="${countAllStaffs}" var="countAllStaffs">			
					...
					${countAllStaffs}
                                   ...
                                   <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12" data-toggle="collapse" data-target="#coll">
                                     ...
                                     <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12" data-toggle="collapse" data-target="#coll">
                                      ...
                                      <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12" data-toggle="collapse" data-target="#coll">
                                        <a class="dashboard-stat bg-primary" href="${pageContext.request.contextPath }/aitiKaceLecturersApiConsumption/getAllLecturersInfoWithoutAction=VIEW=ADMIN-JJD-DN-OSPS-PAGE-Jkdiso=dDFSFUJD-DNDNjfdhnd=dkdoeyrijfd-cnfsd">
                     ...
                     <!--             
					  ...
					  Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/java_project_trials?useSSL=false", "root", "");                     

					  Statement stmt = conn.createStatement();
					  ResultSet rs = stmt.executeQuery("SELECT count(id)  FROM lecturer_profile ORDER BY id");                     
...
<br><br>
   ...
   <br><br>
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
           ...
           <div class="container">
		<button type="button" class="btn btn-info float-left" data-toggle="modal" data-target="#myModal">Add Staff</button>
		...
		<div class="modal fade" id="myModal" role="dialog">
			<div class="modal-dialog">
				...
				<div class="modal-content">
					<div class="modal-header">
					<h4 class="modal-title text-primary">ADD NEW STAFF</h4>
	...
	<form action="${pageContext.request.contextPath }/aitiKaceModulesCoursesApiConsumption/saveStaffs" method="post">
						...
						<div class="row">
								...
								<label>STAFF NAME</label>
								 <input name="name"
										type="text" class="form-control" id="name" value="" placeholder="Staff Name" required>
							...
							<label>STAFF ID</label>
								<div class="form-group">
									<input name="staff_id" type="text" class="form-control checking_staff_id" id="staff_id" value="" placeholder="Staff ID" required>
									<span class="error_staff_id" id="staff_id_error_message" style="color:red;"></span>
								</div>
    ...
    <div class="col">
								...
								<div class="form-group">
								<label>PHONE NUMBER</label>	<input name="contact" type="text"
 ...
 <div class="col">
								<div class="form-group">
									...
									<select class="form-control" name="gender">
										...
										<option value="Course Cordinator">Course Cordinator</option>
...
<option value="Deactivate">Deactivate</option>		     
        ...
        <div class="container">
		<button type="button" class="btn btn-info float-left" data-toggle="modal" data-target="#courseAndModule">Add Course and Module</button>
		...
		<div class="modal fade" id="courseAndModule" role="dialog">
			<div class="modal-dialog">
				...
				<div class="modal-content">
					<div class="modal-header">
					<h4 class="modal-title text-primary">ADD COURSE AND MODULE</h4>
						<button type="button" class="close" data-dismiss="modal">&times;</button>						
					</div>
					<div class="modal-body">
						...
						<form action="${pageContext.request.contextPath }/aitiKaceModulesCoursesApiConsumption/saveCoursesModules" method="post">
			...
			<div class="modal-body">
						...
						<div class="row">
   							 <div class="col">
   							 	...
   							 	<div class="form-group">
									<label>COURSE</label> 
									<select name="course_name" id="course_name" class="form-control" onchange="showCoursesModlues(this.id, 'modules')" required="required">
										<option value="">Select Course</option>
										<option value="Certificate in Software Development(CSD)">Certificate in Software Development(CSD)</option>
										<option value="Cisco Certified Network Associate(CCNA)">Cisco Certified Network Associate(CCNA)</option>
										<option value="Cisco Certified Network Professionals">Cisco Certified Network Professionals(CCNP)</option>
										<option value="International Computer Driving Licence(ICDL)">International Computer Driving Licence(ICDL)</option>
										<option value="Cyber Security">Cyber Security</option>
										<option value="Network Information Security">Network Information Security(NIS)</option>
   ...
   <div class="col">
  	...
  	<select name="module_name" id=modules class="form-control" required="required">
...
<option value="">Select Staff</option>
        		...
        		<c:forEach items="${retrieveOfAllStaffName}" var="retrieveOfAllStaffName">			
				
				   <option value="${retrieveOfAllStaffName.name }"> 
			
					${retrieveOfAllStaffName.name }
...
<div class="card shadow mb-4">
  ...
  <div class="card-body">
	...
	<tbody>
			...
			<c:forEach items="${findAllCourses}" var="findAllCourses">

				<c:url value="updateCoursesModules" var="updateCoursesModulesURL">
					<c:param name="course_name" value="${findAllCourses.course_name }"></c:param>
					<c:param name="course_level" value="${findAllCourses.course_level }"></c:param>
					<c:param name="course_start_date" value="${findAllCourses.course_start_date }"></c:param>
					<c:param name="course_end_date" value="${findAllCourses.course_end_date }"></c:param>
					<c:param name="status" value="${findAllCourses.status }"></c:param>
					<c:param name="created_at" value="${findAllCourses.created_at }"></c:param>
					<c:param name="updated_at" value="${findAllCourses.updated_at }"></c:param>
					...
					<c:param name="id" value="${findAllCourses.id }"></c:param>
				</c:url>


				<tr>
				
					<td>${findAllCourses.id }</td>
					<td>${findAllCourses.course_name }</td>
					<td>${findAllCourses.course_level }</td>
					<td>${findAllCourses.course_start_date }</td>
					<td>${findAllCourses.course_end_date }</td>
					<td>${findAllCourses.status }</td>
					<td>${findAllCourses.created_at }</td>
					<td>${findAllCourses.updated_at }</td>
					
					
					<td><a href="${updateCoursesModulesURL }"><i class="fa fa-edit color-primary" title="Deactivate"></i></a>
					<!-- 
...
<br><br><br><br><br><br>
	...
	<tbody>
			...
			<c:forEach items="${findAllModules}" var="findAllModules">

				<c:url value="updateCoursesModules" var="updateCoursesModulesURL">
				<c:param name="module_name" value="${findAllModules.module_name }"></c:param>	
					<c:param name="staff_name" value="${findAllModules.staff_name }"></c:param>
					<c:param name="status" value="${findAllModules.status }"></c:param>
					<c:param name="course_id" value="${findAllModules.course_id }"></c:param>
					<c:param name="created_at" value="${findAllModules.created_at }"></c:param>
					<c:param name="updated_at" value="${findAllModules.updated_at }"></c:param>
					...
					<c:url value="deleteCoursesModules" var="deleteCoursesModulesURL">
					<c:param name="id" value="${findAllModules.id }"></c:param>
				</c:url>


				<tr>
				
					<td>${findAllModules.id }</td>
					<td>${findAllModules.module_name }</td>
					<td>${findAllModules.staff_name }</td>
					<td>${findAllCoursesModules.status }</td>
					<td>${findAllCoursesModules.created_at }</td>
					<td>${findAllCoursesModules.updated_at }</td>
...
<div class="card shadow mb-4">
  <div class="card-header py-3">
    <h6 class="m-0 font-weight-bold text-primary">AITI-KACE STAFFS DATATABLE</h6>
  </div>
  <div class="card-body">
    <div class="table-responsive">


<table id="examples" class="table table-striped table-bordered"  width="100%" cellspacing="0">
	...
	<tbody>
			...
			<c:forEach items="${findAllStaffNames}" var="findAllStaffNames">



				<tr>
				
					<td>${findAllStaffNames.id }</td>
					<td>${findAllStaffNames.name }</td>
					<td>${findAllStaffNames.staff_id }</td>
					<td>${findAllStaffNames.email }</td>
					<td>${findAllStaffNames.contact }</td>
					<td>${findAllStaffNames.course }</td>
					<td>${findAllStaffNames.usertype }</td>
					<td>${findAllStaffNames.status }</td>
					<td>${findAllStaffNames.created_at }</td>
					<td>${findAllStaffNames.updated_at }</td>
...
function showCoursesModlues(s1,s2) {
	...
	if(s1.value == "Certificate in Software Development(CSD)") {
		
    //var optionArray = url: "getAllAitiKaceStudentNames";
	var optionArray = ['foc|Foc', 'html|Html', 'javascript|JavaScript', , 'Oracle Database|Oracle Database', , 'Software Engineering|Software Engineering', , 'Python|Python'];	
		
	}
	
	else if(s1.value == "Cisco Certified Network Associate(CCNA)") {
		
		var optionArray = ['network|Network', 'cable|Cable', 'cisco|cisco'];
	}
	
	else if(s1.value == "International Computer Driving Licence(ICDL)") {
		
		var optionArray = ['Microsoft Word|Microsoft Word', 'Excel|Excel', 'IT Security|IT Security', 'PowerPoint|PowerPoint'];
	}
